    ...
    req.on('end', async () => {
      try {
        ...
        const msg = await anthropic.messages.create({
          ...
          maxTokens: 50,
          messages: [{ role: "user", content: `Classify the sentiment of the following sentence strictly as either "Positive" or "Negative": "${text}"` }]
        });
    ...
    res.end(`
      ...
      <head>
        ...
        <style>
          ...
          .card { background: #1e293b; padding: 2rem; border-radius: 1rem; box-shadow: 0 10px 25px rgba(0,0,0,0.3); width: 100%; max-width: 400px; }
